...
CSV rows are sorted by default by `percentage_deleted` descending, `name` A-Z, then `deleted_reviews_max` descending. Disable this for a run with:

```bash
npm start -- --city Köln --search-term Hotel --depth 100 --no-sort-csv
```
...
Create a deduplicated merged CSV from a batch run:

```bash
npm start -- --city Bonn --country Germany --search-terms restaurant,Cafe,Hotel --depth 50 --merge-csv-path output/deleted-reviews-bonn-merged.csv
```

Duplicate venues in the merged file are detected by venue name; the first occurrence is kept.
...
Sort CSV files that were generated by older versions:

```bash
npm run sort-csv -- output/deleted-reviews-berlin-doner.csv
```

You can pass multiple files in one call.

Merge CSV files that were generated by older versions:

```bash
npm run merge-csv -- output/deleted-reviews-merged.csv output/deleted-reviews-berlin-doner.csv output/deleted-reviews-berlin-kebab.csv
```
